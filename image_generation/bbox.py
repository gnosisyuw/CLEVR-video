...
parser.add_argument('--output_image_dir', default='../outputv7/images/',
                    help="The directory where output images will be stored. It will be " +
                         "created if it does not exist.")
parser.add_argument('--output_scene_dir', default='../outputv7/scenes/',
                    help="The directory where output JSON scene structures will be stored. " +
                         "It will be created if it does not exist.")
parser.add_argument('--output_scene_file', default='../outputv7/CLEVR_scenes.json',
                    help="Path to write a single JSON file containing all scene information")
parser.add_argument('--output_blend_dir', default='outputv7/blendfiles',
...
def render_scene(args,
    ...
    # ds = bpy.context.objects




    bpy.data.objects[reference[-1]].select = False
    ...
    all_box = {}
    bbox = []
    for robj in reference:
        me_ob = bpy.data.objects[robj]
        output = camera_view_bounds_2d(bpy.context.scene, cam_ob, me_ob)
        bbox.append([output.x,output.y,output.width,output.height])
    all_box[index] = bbox
    ...
    # bpy.data.objects[reference[0]].select = True
    # bpy.ops.transform.translate(value=(3, -3, 2.4))
    # bpy.ops.transform.rotate(value=1.5708, axis=(1.0, 0.0, 0.0))
    # index += 1
    render_args.filepath = output_image + save_template % (index)
    bpy.ops.render.render(write_still=True)
    ...
    if len(l1) == 2:
        ...
        r, q  = a1//1.57084*1.5708 - a1, (0, 0, 1)



    for _ in range(int(lift_t)):
        index += 1
        bpy.ops.transform.translate(value=lift_vect/float(lift_t))
        bpy.ops.transform.rotate(value=r/float(lift_t), axis=q)
        render_args.filepath = output_image  + save_template % (index)
        bpy.ops.render.render(write_still=True)

        me_ob = bpy.data.objects[reference[0]]
        output = camera_view_bounds_2d(bpy.context.scene, cam_ob, me_ob)
        prev_box = all_box[index-1].copy()
        prev_box[0] = [output.x,output.y,output.width,output.height]
        all_box[index] = prev_box
    ...
    # current_t += drop_t
    for _ in range(int(drop_t)):
        index += 1
        bpy.ops.transform.translate(value=-lift_vect/float(drop_t))
        # bpy.ops.transform.rotate(value=r/float(lift_t), axis=q)
        render_args.filepath = output_image  + save_template % (index)
        bpy.ops.render.render(write_still=True)
    ...
    if len(l2) == 2:
        ...
        l2 = [l2[0] + c2, l2[1] - c2]
    bpy.data.objects[reference[0]].select = False
    bpy.data.objects[reference[1]].select = True
    ...
    if len(l2) == 2:
        ...
        r, q = a2//1.57084*1.5708 - a2, (0, 0, 1)
    ...
    for _ in range(int(lift_t)):
        ...
        me_ob = bpy.data.objects[reference[1]]
        output = camera_view_bounds_2d(bpy.context.scene, cam_ob, me_ob)
        prev_box = all_box[index - 1].copy()
        prev_box[1] = [output.x, output.y, output.width, output.height]
        all_box[index] = prev_box
    ...
    move_vect = item1.location - item2.location + move_list[pos]

    for _ in range(int(move_t)):
        index += 1
        bpy.ops.transform.translate(value=move_vect/float(move_t))
        # bpy.ops.transform.rotate(value=r/float(lift_t), axis=q)
        render_args.filepath = output_image  + save_template % (index)
        bpy.ops.render.render(write_still=True)
    ...
    for _ in range(int(drop_t)):
        index += 1
        bpy.ops.transform.translate(value=drop_list[pos]/float(drop_t))
        # bpy.ops.transform.rotate(value=r/float(lift_t), axis=q)
        render_args.filepath = output_image  + save_template % (index)
        bpy.ops.render.render(write_still=True)
    ...
    bpy.data.objects[reference[1]].select = False
    bpy.data.objects[reference[2]].select = True
    # 3rd align
    if len(l3) == 2:
        if len(att_loc) <= 2:
            c3 = c1
    ...
    if len(l3) == 2:
        ...
        r, q =  a3 // 1.57084 * 1.5708 - a3, (0, 0, 1)
    ...
    for _ in range(int(lift_t)):
        ...
        me_ob = bpy.data.objects[reference[2]]
        output = camera_view_bounds_2d(bpy.context.scene, cam_ob, me_ob)
        prev_box = all_box[index - 1].copy()
        prev_box[2] = [output.x, output.y, output.width, output.height]
        all_box[index] = prev_box
    ...
    move_vect = use_location + move_list[pos] + add_vec - item3.location
    ...
    for _ in range(int(drop_t)):
        index += 1
        bpy.ops.transform.translate(value=(drop_list[pos]- add_vec)/float(drop_t))
        # bpy.ops.transform.rotate(value=r/float(lift_t), axis=q)
        render_args.filepath = output_image  + save_template % (index)
        bpy.ops.render.render(write_still=True)
    ...
    # with open('/home/stevelab2/Documents/stacking/outputv7/temp.json', 'w+') as f:
    #     json.dump(att_loc,f)
    #     json.dump([record,pos, list(item3.location)], f)
    #     json.dump(output_image, f)
    # #     json.dump(new_list,f)

    # with open('/home/wei/PycharmProjects/stacking/outputv7/test.json', 'w') as f:
    #     json.dump(all_box, f, indent=2)

    scene_struct['actions'] = [action_1 ,record, pos]
    scene_struct['bboxes'] = all_box
